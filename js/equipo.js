...
const Equipo = {
  ...
  async nuevoUsuario() {
    ...
    if (!pin || pin.length !== 4 || isNaN(pin)) {
      App.toast('El PIN debe ser de 4 dígitos');
      return;
    }
    ...
    Auth.audit('usuario_creado', null, { nombre, rol: rol.nombre });
    App.toast(`${nombre} agregado al equipo`);
  ...
  async editar(id) {
    ...
    if (cambioRol) {
      Auth.audit('rol_cambiado', id, { nombre, rol_anterior: u.rol, rol_nuevo: rol.nombre }, 'warning');
    }
  ...
  async nuevoRol() {
    ...
    for (const v of vistas) {
      if (confirm(`¿El rol "${nombre}" puede acceder a "${v}"?`)) {
        permisos.push(v);
      }
    }

    const esAdmin = confirm(`¿"${nombre}" es administrador? (acceso total + superpoderes)`);

    await SB.insertN('taq_roles', { nombre, permisos, es_admin: esAdmin });
    App.toast(`Rol "${nombre}" creado`);
    this.render(document.getElementById('main'));
  },

  async editarRol(id) {
    const [rol] = await SB.get('taq_roles', `id=eq.${id}`);
    if (!rol) return;
    ...
    for (const v of vistas) {
      ...
      if (confirm(`¿"${nombre}" puede acceder a "${v}"?${tiene ? ' (actualmente SÍ)' : ' (actualmente NO)'}`)) {
        permisos.push(v);
      }
    }

    const esAdmin = confirm(`¿"${nombre}" es administrador?${rol.es_admin ? ' (actualmente SÍ)' : ' (actualmente NO)'}`);

    await SB.update('taq_roles', `id=eq.${id}`, { nombre, permisos, es_admin: esAdmin });
    App.toast(`Rol "${nombre}" actualizado`);
    this.render(document.getElementById('main'));
  }
};
